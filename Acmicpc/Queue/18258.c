...
int main()
{
	...
	int * printArray = (int *)malloc(sizeof(int) * n);
	if(printArray == NULL) return 0;
	...
	for(int i = 0; i < n; i++)
	{
		...
		else
		{
			...
			{
				printArray[count] = list->front(list);
			}
			else if(strncmp(buffer, "back", 4) == 0)
			{
				printArray[count] = list->back(list);
			}
			else if(strncmp(buffer, "size", 4) == 0)
			{
				printArray[count] = list->size(list);
			}
			else if(strncmp(buffer, "pop", 3) == 0)
			{
				printArray[count] = list->pop(list);
			}
			else if(strncmp(buffer, "empty", 5) == 0)
			{
				printArray[count] = list->isEmpty(list);
			}
